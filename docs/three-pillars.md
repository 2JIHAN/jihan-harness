...
- **배치 및 배선**
  ...
  - 카탈로그: `.agents/skills/INDEX.md` (`.agents/AGENTS.md`에서 `@skills/INDEX.md`로 상시 임포트)
...
이 모순을 해결하기 위해 **스킬 묶음 안에 가벼운 카탈로그 한 장을 두고, 본문은 각 스킬에 격리하는 라우터 패턴**을 적용한다:

- **카탈로그 (`skills/INDEX.md`)** — 스킬 이름과 발동 조건, 본문 위치를 한 행씩 담은 표만 상시 주입한다. 필수 스킬(코딩 시 `ponytail`, 디버깅 시 `systematic-debugging`, 한국어 문서 작성 시 `writing-docs-in-korean`)은 행에 표시하여 파일을 건드리기 전에 반드시 적재하도록 강제한다.
- **본문 (`skills/*/SKILL.md`)** — 100~300줄에 달하는 구체적 방법론과 레퍼런스는 온디맨드로 유지한다.
- **라우터를 Rule이 아니라 Skill에 두는 이유** — 라우팅 표는 스킬 목록이 바뀔 때마다 함께 바뀐다. 표를 `rules/`에 두면 스킬을 추가하거나 삭제할 때 두 기둥을 동시에 고쳐야 하고, 둘이 어긋나면 존재하지 않는 스킬을 가리키게 된다. 카탈로그를 스킬 묶음 안에 두면 스킬과 그 색인이 한 디렉터리에서 함께 움직인다.

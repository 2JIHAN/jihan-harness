...
- **참조 줄 중복 방지** — `AGENTS.md`에 `@` 참조를 추가할 때 `grep -qF` 검사를 거쳐 중복 라인 누적을 차단한다.
- **안전한 파일 갱신** — `cp` 또는 `ln -sf` 실행 시 기존 대상이 존재하더라도 오류 없이 갱신한다.
...
- **스킬 심링크 배선** — `.claude/skills`를 `../.agents/skills`로 연결하는 심링크를 자동으로 생성한다.
- **Git 훅 오버라이드** — `git config core.hooksPath .git/hooks` 설정을 주입하여 전역 설정을 덮어쓴다.
...
- **런타임 무의존** — `npm install`, `pip install` 과정 없이 OS 기본 도구(`bash`, `git`, `node`)로만 실행한다.
- **계층 간 무의존**
  - Rule은 Skill 존재 여부에 의존하지 않는다.
  - Rule 간 상호 참조를 하지 않는다 (`fluent-korean.md`와 `terminal-response-format.md`의 완전한 분리).
  - Hook은 에이전트 설치 여부와 무관하게 Git 환경에서 단독으로 실행된다.
- **단위별 독립 배포** — 전체 설치 외에 특정 단위만 독립적으로 배포할 수 있다 (`--rules`, `--skills`, `--hooks`, `npx skills add`).
